...
function ENT:Draw()
	...
	pos = pos + oang:Forward() + oang:Up() * 85  + oang:Right() * 0

	if alpha > 0 then
		cam.Start3D2D( pos, ang, 0.025 )
			draw.SimpleText( self:GetTitle() or 'Необъявленная местность', 'font_sans_3d2d_large', 0, 0, Color(225, 177, 44, alpha), TEXT_ALIGN_CENTER )
			draw.SimpleText( self:GetClaimers() ~= 'Free' and 'Занято: '..self:GetClaimers() or 'Свободно', 'font_sans_3d2d_small', 0, 128, Color(255,255,255, alpha), TEXT_ALIGN_CENTER )			
