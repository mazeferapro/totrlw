...
classes.BarHover = function(pnl, col, height, speed, place)
	col = col or Color(255, 255, 255, 255)
	
	height = height or 2
	place = place or height
	speed = speed or 6

	pnl:SetupTransition('m_BarHover', speed, TDLibUtil.HoverFunc)
	pnl:On('PaintOver', function(s, w, h)
		local bar = math.Round(w*s.m_BarHover)

		surface.SetDrawColor(col)
		surface.DrawRect(w * 0.5-bar * 0.5, h-place, bar, height)
	end)
end
